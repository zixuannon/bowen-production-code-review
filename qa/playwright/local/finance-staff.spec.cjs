...
test('School Admin manages Finance roles and Cashier Fund Accounts through application modals without native dialogs', async ({ browser }) => {
  ...
  try {
    ...
    const assignCashier = page.waitForResponse(response => response.url().includes('/finance-staff/') && response.url().endsWith('/role') && response.request().method() === 'POST');
    await modal.getByRole('button', { name: 'Assign Cashier' }).click();
    expect((await assignCashier).status()).toBe(200);
    await page.goto('/finance-staff', { waitUntil: 'networkidle' });
    await expect(teacherRow).toContainText('Cashier');

    await teacherRow.getByRole('button', { name: 'Manage' }).click();
    const removeCashier = page.waitForResponse(response => response.url().includes('/finance-staff/') && response.url().endsWith('/role') && response.request().method() === 'POST');
    await modal.getByRole('button', { name: 'Remove Cashier' }).click();
    expect((await removeCashier).status()).toBe(200);
    await page.goto('/finance-staff', { waitUntil: 'networkidle' });
