...
test('GROUP_QA HQ report renders only member Schools and exports canonical Ledger V1 rows', async ({ browser }) => {
  ...
  try {
    ...
    await expect(page.getByRole('cell', { name: 'Zixuan QA School', exact: true }).first()).toBeVisible();
    await expect(page.getByRole('cell', { name: 'Timecity QA School', exact: true }).first()).toBeVisible();
