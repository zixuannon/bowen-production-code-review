...
or create financial records. `FinanceGroupReportService` is now the scoped
...
Bank Account list cash-flow aggregation follow-up — **LOCAL ACCEPTANCE VERIFIED**. The paginated Fund Account list now reuses `FundAccountBalanceService` bulk cash-flow summaries: Money In is operating receipts plus incoming completed BankTransfer, while Money Out is Expense plus outgoing completed BankTransfer. Confirmed Fund Handover appears exactly once through its canonical BankTransfer; pending Handover has no list-total effect. Internal transfers remain excluded from Operating Income, Operating Expense, and Operating Net. The columns are labeled `Money In` / `Money Out` with `流入合计` / `流出合计` Chinese translations. Focused regression passed (18 tests / 122 assertions), broader Finance regression passed (160 tests / 666 assertions), the full PHPUnit suite passed (349 tests / 1,150 assertions), and guarded local BOWEN_QA Playwright acceptance passed (1/1). Production and staging remain untouched.
