...
Finance P3.2 Unified Finance Transactions — **LOCAL CODE / PHPUNIT VERIFIED; BROWSER QA PENDING**. `Finance → Transactions` is a read-only adapter over compulsory payments, optional payments, non-fee `OtherIncome`, Expense, and canonical completed BankTransfer source records; no duplicate transaction/ledger table exists. Pending handovers are omitted, while confirmed handovers appear exactly once through their linked BankTransfer. The register has date/type/account/reference/keyword filters and rejects forged Fund Account filters server-side. Its operating totals exclude internal transfers; account movement totals retain transfer in/out visibility. Receive Money records a tenant-local non-fee source only after payment-method, active current-school Fund Account, Cashier assignment scope, and reference reservation checks. Expense/Import buttons reuse existing Expense and P3.1 Expense Excel workflows. Focused tests passed (25 tests / 108 assertions); broader P1–P3.2 Finance regression passed (131 tests / 481 assertions). BOWEN_QA browser acceptance remains blocked without bypass: local reset is refused because `forge@localhost` cannot create `eschool_local_bowen_qa`, `@playwright/test` is unavailable, and no local server is listening on 127.0.0.1:8000. Production and staging remain untouched.
