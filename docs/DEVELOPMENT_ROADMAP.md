...
- [x] Bank Account list cash-flow follow-up: per-account Money In/Out includes directional completed BankTransfers through the canonical Fund Account balance service; confirmed Handovers count once through their linked BankTransfer, pending Handovers remain neutral, and operating Income/Expense/Net are unchanged. Labels are `Money In` / `Money Out` (`流入合计` / `流出合计`). Focused regression: 18 tests / 122 assertions; Finance regression: 160 tests / 666 assertions; guarded local Playwright: 1/1.
