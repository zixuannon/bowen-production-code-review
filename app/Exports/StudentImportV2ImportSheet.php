...
final class StudentImportV2ImportSheet implements FromArray, WithColumnWidths, WithEvents, WithHeadings, WithStrictNullComparison, WithTitle
{
    ...
    public function registerEvents(): array
    {
        return [AfterSheet::class => function (AfterSheet $event): void {
            ...
            // Source references and phone numbers are business identifiers, not numbers.
            foreach (['A', 'F', 'G'] as $column) $sheet->getStyle("{$column}2:{$column}{$lastRow}")->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
            foreach (['I', 'J'] as $column) $sheet->getStyle("{$column}2:{$column}{$lastRow}")->getNumberFormat()->setFormatCode('yyyy-mm-dd');
            ...
            $this->comment($sheet, 'A1', 'Required stable source reference. Student Code is generated by the server and must not be entered here.');
            $this->comment($sheet, 'F1', 'Required. Text format preserves leading zeroes, for example 0912345678.');
            $this->comment($sheet, 'G1', 'Optional. Text format preserves leading zeroes, for example 0912345678.');
            $this->comment($sheet, 'C1', 'Choose a Class Section for this School. Server-side ownership validation remains authoritative.');
            $this->comment($sheet, 'D1', 'Choose Weekday or Weekend.');
            for ($row = 2; $row <= $lastRow; $row++) {
                $this->list($sheet, "C{$row}", '=StudentImportClassSections');
                $this->list($sheet, "D{$row}", '"Weekday,Weekend"');
                $this->list($sheet, "H{$row}", '=StudentImportGenders');
                $this->list($sheet, "K{$row}", '"Active,Inactive"');
