...
<select name="fund_account_id" class="form-control mb-2" required><option value="">{{ __('Fund Account') }}</option>@foreach($accounts as $account)<option value="{{ $account->id }}">{{ $account->account_name }}</option>@endforeach</select>
