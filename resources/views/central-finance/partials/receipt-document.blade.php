...
<h6>{{ __('Fund Account') }}</h6><table class="table table-sm table-bordered"><tbody>
    ...
    <tr><th>{{ __('Account type') }}</th><td>{{ __($receipt->fundAccount['type']) }}</td></tr><tr><th>{{ __('Bank / identifier') }}</th><td>{{ collect([$receipt->fundAccount['bank_name'],$receipt->fundAccount['masked_identifier']])->filter()->join(' · ') ?: '—' }}</td></tr><tr><th>{{ __('Currency') }}</th><td>{{ $receipt->fundAccount['currency'] }}</td></tr>
</tbody></table>
