...
<div class="content-wrapper">
    ...
    <div class="row">
        ...
        <main class="col-lg-9 col-xl-10">
            ...
            @if($page === '__legacy_staff_template')
                <div class="card"><div class="card-body"><h5>{{ __('Central Finance Staff') }}</h5><p class="text-muted">{{ __('School Scope is configured by Central Super Admin in Finance Groups. Fund Account Scope is configured by Head Finance from the relevant Fund Account.') }}</p>@if(!$canConfigureAccounts)<div class="alert alert-warning mb-0">{{ __('Select a School where you are authorized as Head Finance to view configuration details.') }}</div>@else<div class="table-responsive"><table class="table mb-0"><thead><tr><th>{{ __('Name') }}</th><th>{{ __('Email') }}</th><th>{{ __('School Scope') }}</th><th>{{ __('Fund Account Scope') }}</th></tr></thead><tbody>@foreach($staff as $member)<tr><td>{{ $member->full_name }}</td><td>{{ $member->email }}</td><td>{{ __('View / operate as granted by Central Super Admin') }}</td><td>{{ $member->authorizedFundAccounts->pluck('account_name')->implode(', ') ?: __('None') }}</td></tr>@endforeach</tbody></table></div>@if(method_exists($staff,'links'))<div class="mt-3">{{ $staff->links() }}</div>@endif@endif</div></div>
            @endif
            @endif
