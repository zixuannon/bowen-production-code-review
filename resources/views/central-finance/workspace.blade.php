...
<div class="content-wrapper">
    ...
    <div class="row">
        <aside class="col-lg-3 col-xl-2">
            <div class="card"><div class="card-body p-2 central-finance-nav">
                <nav class="nav nav-pills flex-column" aria-label="{{ __('Central Finance navigation') }}">
                    ...
                    <a class="nav-link {{ $page === 'payments' ? 'active' : '' }}" href="{{ route('central-finance.payments.index') }}">{{ __('收费记录 / 收据') }}</a>
                    <a class="nav-link {{ $page === 'student-ledger' ? 'active' : '' }}" href="{{ route('central-finance.student-ledger') }}">{{ __('学生账本') }}</a>
                    ...
                    @if($canConfigureAccounts)<a class="nav-link {{ $page === 'audits' ? 'active' : '' }}" href="{{ route('central-finance.audits') }}">{{ __('审计记录') }}</a>@endif
                    @if($canConfigureAccounts)<span class="nav-label">{{ __('Central Finance 设置') }}</span><a class="nav-link {{ $page === 'accounts' ? 'active' : '' }}" href="{{ route('central-finance.accounts') }}#central-finance-setup">{{ __('账户与开账设置') }}</a><a class="nav-link {{ $page === 'staff' ? 'active' : '' }}" href="{{ route('central-finance.staff') }}">{{ __('财务人员') }}</a><a class="nav-link {{ $page === 'categories' ? 'active' : '' }}" href="{{ route('central-finance.categories') }}">{{ __('财务分类') }}</a>@endif
                </nav>
            </div></div>
        </aside>
        ...
        <main class="col-lg-9 col-xl-10">
            ...
            @if($page === 'receivables')
                <div class="card central-finance-form-card mb-3"><div class="card-body"><h5>{{ __('学生收费') }}</h5><p class="text-muted">{{ __('Student identity is read only from Central Student Financial Profiles synchronized from the School tenant.') }}</p>
                    ...
                    @elseif($canOperate)
                        ...
                        <form method="POST" action="{{ route('central-finance.payments.store') }}" id="central-payment-form">@csrf
                            ...
                            <div class="form-group"><label>{{ __('Outstanding Receivable') }}</label><select name="receivable_id" id="central-payment-receivable" class="form-control" required disabled><option value="">{{ __('Select Student first') }}</option>@foreach($paymentReceivables as $r)<option value="{{ $r->id }}" data-student-id="{{ $r->student_profile_id }}" data-outstanding="{{ $r->amount_due - $r->amount_paid }}">{{ $r->description }} · {{ number_format($r->amount_due - $r->amount_paid,2) }} {{ $r->currency }}</option>@endforeach</select><small id="central-payment-empty" class="form-text text-muted d-none">{{ __('当前没有待缴项目') }}</small></div>
                        ...
                        <script>document.addEventListener('DOMContentLoaded',function(){const student=document.getElementById('central-payment-student'),receivable=document.getElementById('central-payment-receivable'),amount=document.getElementById('central-payment-amount'),submit=document.getElementById('central-payment-submit'),empty=document.getElementById('central-payment-empty'),summary=document.getElementById('central-payment-student-summary');if(!student||!receivable)return;const options=[...receivable.querySelectorAll('option[data-student-id]')],fmt=value=>new Intl.NumberFormat(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(value||0));student.addEventListener('change',function(){const id=this.value,selected=this.options[this.selectedIndex];receivable.value='';receivable.disabled=!id;amount.value='';amount.disabled=true;submit.disabled=true;let count=0,outstanding=0;options.forEach(option=>{const visible=option.dataset.studentId===id;option.hidden=!visible;option.disabled=!visible;if(visible){count++;outstanding+=Number(option.dataset.outstanding||0);}});if(id){document.getElementById('central-payment-summary-name').textContent=selected.dataset.name||'';document.getElementById('central-payment-summary-admission').textContent=selected.dataset.admissionNo||'';document.getElementById('central-payment-summary-class').textContent=selected.dataset.class||'—';document.getElementById('central-payment-summary-due').textContent=fmt(selected.dataset.due);document.getElementById('central-payment-summary-paid').textContent=fmt(selected.dataset.paid);document.getElementById('central-payment-summary-outstanding').textContent=fmt(outstanding);document.getElementById('central-payment-summary-items').textContent=count;summary.classList.remove('d-none');}else summary.classList.add('d-none');empty.classList.toggle('d-none',!id||count>0);});receivable.addEventListener('change',function(){const option=this.options[this.selectedIndex];const enabled=!!this.value;amount.disabled=!enabled;submit.disabled=!enabled;if(enabled)amount.max=option.dataset.outstanding;});});</script>
                    @else
                        <div class="alert alert-warning mb-0">{{ __('This School is read-only until Central cutover and operating scope are both active.') }}</div>
                    @endif
                </div></div>
                <div class="card"><div class="card-body"><h5>{{ __('Receivables') }}</h5><div class="table-responsive"><table class="table mb-0"><thead><tr><th>{{ __('Student') }}</th><th>{{ __('Description') }}</th><th>{{ __('Due') }}</th><th>{{ __('Paid') }}</th><th>{{ __('Status') }}</th></tr></thead><tbody>@foreach($receivables as $r)<tr><td>{{ $r->studentProfile?->student_name }}</td><td>{{ $r->description }}</td><td>{{ number_format($r->amount_due,2) }}</td><td>{{ number_format($r->amount_paid,2) }}</td><td>{{ $r->status }}</td></tr>@endforeach</tbody></table></div>@if(method_exists($receivables,'links'))<div class="mt-3">{{ $receivables->links() }}</div>@endif</div></div>
            @endif
            ...
            @if($page === 'payments')
                <div class="card"><div class="card-body"><h5>{{ __('收费记录 / 收据') }}</h5><form method="GET" class="form-row mb-3"><div class="col-md-4"><input class="form-control" name="student" value="{{ $filters['student'] ?? '' }}" placeholder="{{ __('Student name or admission number') }}"></div><div class="col-md-4"><input class="form-control" name="reference" value="{{ $filters['reference'] ?? '' }}" placeholder="{{ __('Payment reference') }}"></div><div class="col-md-2"><button class="btn btn-outline-primary">{{ __('Filter') }}</button></div></form><div class="table-responsive"><table class="table mb-0"><thead><tr><th>{{ __('Date') }}</th><th>{{ __('Student') }}</th><th>{{ __('Amount') }}</th><th>{{ __('Fund Account') }}</th><th>{{ __('Reference') }}</th><th>{{ __('Receipt') }}</th></tr></thead><tbody>@foreach($payments as $payment)<tr><td>{{ $payment->paid_at }}</td><td>{{ $payment->receivable?->studentProfile?->student_name }}</td><td>{{ number_format($payment->amount,2) }} {{ $payment->currency }}</td><td>{{ $payment->fundAccount?->account_name }}</td><td>{{ $payment->payment_reference }}</td><td>{{ $payment->receipt?->receipt_no }}</td></tr>@endforeach</tbody></table></div>@if(method_exists($payments,'links'))<div class="mt-3">{{ $payments->links() }}</div>@endif</div></div>
            @endif
            ...
            @if($page === 'account-report' && $accountReport)
                <div class="card"><div class="card-body"><h5>{{ __('Fund Account Report') }} · {{ $accountReport->account_name }}</h5><p class="text-muted">{{ __('Opening balance') }}: {{ number_format($accountReport->opening_balance,2) }} {{ $accountReport->currency }}</p><div class="table-responsive"><table class="table mb-0"><thead><tr><th>{{ __('Date') }}</th><th>{{ __('Source') }}</th><th>{{ __('Reference') }}</th><th>{{ __('Money In') }}</th><th>{{ __('Money Out') }}</th><th>{{ __('Operating Income') }}</th><th>{{ __('Operating Expense') }}</th></tr></thead><tbody>@foreach($ledger as $entry)<tr><td>{{ $entry->entry_date }}</td><td>{{ $entry->source_type }}</td><td>{{ $entry->reference_no }}</td><td>{{ number_format($entry->money_in,2) }}</td><td>{{ number_format($entry->money_out,2) }}</td><td>{{ number_format($entry->operating_income,2) }}</td><td>{{ number_format($entry->operating_expense,2) }}</td></tr>@endforeach</tbody></table></div>{{ $ledger->links() }}</div></div>
            @endif

            @if($page === 'audits')
                <div class="card"><div class="card-body"><h5>{{ __('审计记录') }}</h5><form method="GET" class="form-row mb-3"><div class="col-md-4"><input class="form-control" name="source" value="{{ $filters['source'] ?? '' }}" placeholder="{{ __('Document type') }}"></div><div class="col-md-2"><button class="btn btn-outline-primary">{{ __('Filter') }}</button></div></form><div class="table-responsive"><table class="table mb-0"><thead><tr><th>{{ __('When') }}</th><th>{{ __('Document') }}</th><th>{{ __('Action') }}</th><th>{{ __('Actor') }}</th><th>{{ __('Reason') }}</th></tr></thead><tbody>@foreach($audits as $audit)<tr><td>{{ $audit->created_at }}</td><td>{{ $audit->document_type }} #{{ $audit->document_id }}</td><td>{{ $audit->action }}</td><td>{{ $audit->actor_id }}</td><td>{{ $audit->reason }}</td></tr>@endforeach</tbody></table></div>@if(method_exists($audits,'links'))<div class="mt-3">{{ $audits->links() }}</div>@endif</div></div>
            @endif

            @if($page === 'ledger')
                <div class="card"><div class="card-body"><h5>{{ __('Standard Ledger') }}</h5>
                    <form method="GET" class="form-row mb-3">
                        <div class="col-md-2"><input type="date" class="form-control" name="from" value="{{ $filters['from'] ?? '' }}"></div>
                        <div class="col-md-2"><input type="date" class="form-control" name="to" value="{{ $filters['to'] ?? '' }}"></div>
                        @if(!$school)<div class="col-md-2"><select class="form-control" name="school_id"><option value="">{{ __('All Schools') }}</option>@foreach($schools as $availableSchool)<option value="{{ $availableSchool->id }}" @selected(($filters['school_id'] ?? null) == $availableSchool->id)>{{ $availableSchool->name }}</option>@endforeach</select></div>@endif
                        <div class="col-md-2"><select class="form-control" name="fund_account_id"><option value="">{{ __('All accounts') }}</option>@foreach($accounts as $account)<option value="{{ $account->id }}" @selected(($filters['fund_account_id'] ?? null) == $account->id)>{{ $account->account_name }}</option>@endforeach</select></div>
                        <div class="col-md-2"><select class="form-control" name="category_id"><option value="">{{ __('All categories') }}</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected(($filters['category_id'] ?? null) == $category->id)>{{ $category->type }} · {{ $category->name }}</option>@endforeach</select></div>
                        <div class="col-md-2"><select class="form-control" name="operator_id"><option value="">{{ __('All operators') }}</option>@foreach($operators as $operator)<option value="{{ $operator->id }}" @selected(($filters['operator_id'] ?? null) == $operator->id)>{{ $operator->full_name }}</option>@endforeach</select></div>
                        <div class="col-md-2"><input class="form-control" name="source" value="{{ $filters['source'] ?? '' }}" placeholder="{{ __('Source type') }}"></div>
                        <div class="col-md-2 mt-2"><button class="btn btn-outline-primary">{{ __('Filter') }}</button></div>
                    </form>
                    <div class="table-responsive"><table class="table mb-0"><thead><tr><th>{{ __('Date') }}</th><th>{{ __('School') }}</th><th>{{ __('Source') }}</th><th>{{ __('Fund Account') }}</th><th>{{ __('Money In') }}</th><th>{{ __('Money Out') }}</th><th>{{ __('Operating Income') }}</th><th>{{ __('Operating Expense') }}</th><th>{{ __('Operator') }}</th></tr></thead><tbody>@foreach($ledger as $entry)<tr><td>{{ $entry->entry_date }}</td><td>{{ $entry->school_id }}</td><td>{{ $entry->source_type }} · {{ $entry->reference_no }}</td><td>{{ $entry->fund_account_id }}</td><td>{{ number_format($entry->money_in,2) }}</td><td>{{ number_format($entry->money_out,2) }}</td><td>{{ number_format($entry->operating_income,2) }}</td><td>{{ number_format($entry->operating_expense,2) }}</td><td>{{ $entry->created_by }}</td></tr>@endforeach</tbody></table></div>{{ $ledger->links() }}
                </div></div>
            @endif
        </main>
    </div>
