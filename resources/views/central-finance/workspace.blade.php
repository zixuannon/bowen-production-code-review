                ...
                <div class="card central-finance-form-card mb-3"><div class="card-body"><h5>{{ __('学生收费') }}</h5><p class="text-muted">{{ __('Student identity is read only from Central Student Financial Profiles synchronized from the School tenant.') }}</p>
                    ...
                    @elseif($canOperate)
                        <form method="POST" action="{{ route('central-finance.payments.store') }}" id="central-payment-form">@csrf
                            <div class="form-group"><label>{{ __('Student') }}</label><select id="central-payment-student" class="form-control" required><option value="">{{ __('Select Student') }}</option>@foreach($paymentProfiles as $profile)<option value="{{ $profile->id }}">{{ $profile->student_name }}{{ $profile->admission_no ? ' · '.$profile->admission_no : '' }}</option>@endforeach</select></div>
                        ...
                        <script>document.addEventListener('DOMContentLoaded',function(){const student=document.getElementById('central-payment-student'),receivable=document.getElementById('central-payment-receivable'),amount=document.getElementById('central-payment-amount'),submit=document.getElementById('central-payment-submit'),empty=document.getElementById('central-payment-empty');if(!student||!receivable)return;const options=[...receivable.querySelectorAll('option[data-student-id]')];student.addEventListener('change',function(){const id=this.value;receivable.value='';receivable.disabled=!id;amount.value='';amount.disabled=true;submit.disabled=true;let count=0;options.forEach(option=>{const visible=option.dataset.studentId===id;option.hidden=!visible;option.disabled=!visible;if(visible)count++;});empty.classList.toggle('d-none',!id||count>0);});receivable.addEventListener('change',function(){const option=this.options[this.selectedIndex];const enabled=!!this.value;amount.disabled=!enabled;submit.disabled=!enabled;if(enabled)amount.max=option.dataset.outstanding;});});</script>
                    @else
