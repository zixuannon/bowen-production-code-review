<div class="form-row">
    ...
    <div class="form-group col-md-4"><label for="coa-code-{{ $formKey }}">{{ __('Account Code') }}</label><input id="coa-code-{{ $formKey }}" type="text" name="category_code" class="form-control" value="{{ $coa?->category_code }}" maxlength="80" required placeholder="0101">@if($coa)<small class="text-muted">{{ __('Account Code may be corrected with an audit reason. Historical records keep their existing snapshots.') }}</small>@endif</div>
    ...
    <div class="form-group col-md-8"><label for="coa-english-name-{{ $formKey }}">{{ __('English display name') }}</label><input id="coa-english-name-{{ $formKey }}" name="english_name" class="form-control" value="{{ $coa ? app(\App\Services\CentralFinanceBusinessContentTranslationService::class)->english('chart_account', $coa->id, null) : '' }}" maxlength="2000"><small class="text-muted">{{ __('Optional. English falls back to the original name; changing the original marks this translation for review.') }}</small></div>
