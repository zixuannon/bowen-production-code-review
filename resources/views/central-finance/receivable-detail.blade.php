        ...
        <div class="col-lg-5 mb-3">
            <div class="card cf-danger-panel"><div class="card-body">
                ...
                @elseif($document->status !== \App\Models\CentralFinanceReceivable::CANCELLED)
                    <form method="POST" action="{{ route('central-finance.receivables.adjustments.store', $document->id) }}" data-lifecycle-confirm data-lifecycle-object="{{ $document->studentProfile?->student_name ?: __('Receivable') }} · {{ $document->description }}" data-lifecycle-amount="{{ number_format($document->amount_due, 2) }} {{ $document->currency }}" data-lifecycle-current-status="{{ __($document->status) }}" data-lifecycle-result="{{ __('Audited adjustment, waiver, or void') }}">@csrf
