...
(() => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const body = document.createElement('tbody'); payload.rows.forEach((item) => { const row = document.createElement('tr'); addCell(row, item.line); addCell(row, item.student_code); addCell(row, `${item.first_name || ''} ${item.last_name || ''}`.trim()); addCell(row, item.class_section); addCell(row, item.academic_year); addCell(row, item.status); addCell(row, [...(item.errors || []), ...(item.warnings || [])].join(' ')); body.appendChild(row); }); table.appendChild(body);
