        ...
        <section id="staff" class="card mb-4"><div class="card-body">
            ...
            <div class="table-responsive"><table class="table table-sm"><thead><tr><th>{{ __('User') }}</th><th>{{ __('Role') }}</th><th>{{ __('School') }}</th><th>{{ __('View') }}</th><th>{{ __('Operate') }}</th><th>{{ __('Approve') }}</th><th>{{ __('Confirm') }}</th><th>{{ __('Status') }}</th><th>{{ __('Edit') }}</th><th>{{ __('Revoke') }}</th></tr></thead><tbody>
                @forelse($centralScopes as $scope)
                    ...
                    <tr><td>{{ trim($scope->first_name.' '.$scope->last_name) ?: ('User #'.$scope->user_id) }}</td><td>{{ $centralUser?->hasRole('Head Finance') ? __('Head Finance') : __('School Accountant') }}</td><td>{{ $scope->school_name }}</td><td>{{ $scope->can_view ? '✓' : '—' }}</td><td>{{ $scope->can_operate ? '✓' : '—' }}</td><td>{{ $scope->can_approve_reimbursements ? '✓' : '—' }}</td><td>{{ $scope->can_confirm_funding ? '✓' : '—' }}</td><td>{{ $active ? __('Active') : __('Disabled') }}</td>
                        ...
                        <td><form method="POST" action="{{ route('finance-groups.central-school-scopes.disable', $group) }}">@csrf<input type="hidden" name="central_user_id" value="{{ $scope->user_id }}"><input type="hidden" name="school_id" value="{{ $scope->school_id }}"><button class="btn btn-sm btn-outline-danger">{{ __('Revoke / Disable') }}</button></form></td>
                    </tr>
                @empty
