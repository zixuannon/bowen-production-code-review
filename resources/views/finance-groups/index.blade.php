            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <form class="row" method="POST" action="{{ route('finance-groups.user-scopes.store', $group) }}">@csrf
                        ...
                        <div class="form-group col-md-2"><select class="form-control" name="capability"><option value="view_reports">{{ __('View reports') }}</option><option value="export_reports">{{ __('Export reports') }}</option><option value="operate_finance">{{ __('Operate School Finance') }}</option><option value="manage_configuration">{{ __('Manage configuration') }}</option><option value="manage_hq_accounts">{{ __('Manage HQ Fund Accounts') }}</option><option value="request_group_transfers">{{ __('Request Group funding') }}</option><option value="confirm_group_transfers">{{ __('Confirm Group funding') }}</option></select></div>
                    ...
                    <h5>{{ __('Central Finance School scope') }}</h5>
                    <form class="row align-items-end" method="POST" action="{{ route('finance-groups.central-school-scopes.store', $group) }}">@csrf
                        <div class="form-group col-md-3"><label>{{ __('Central user') }}</label><select class="form-control" name="central_user_id" required><option value="">{{ __('Select central user') }}</option>@foreach($centralUsers as $user)<option value="{{ $user->id }}">{{ $user->full_name }}</option>@endforeach</select></div>
                        ...
                        <div class="form-group col-md-4"><label class="mr-2"><input type="checkbox" name="can_view" value="1" checked> {{ __('View') }}</label><label class="mr-2"><input type="checkbox" name="can_operate" value="1"> {{ __('Operate') }}</label><label class="mr-2"><input type="checkbox" name="can_approve_reimbursements" value="1"> {{ __('Approve reimbursement') }}</label><label><input type="checkbox" name="can_confirm_funding" value="1"> {{ __('Confirm funding') }}</label></div>
                        <div class="form-group col-md-2"><button class="btn btn-outline-primary" type="submit">{{ __('Save Central scope') }}</button></div>
                    </form>
