...
<div class="content-wrapper">
    ...
    <div class="card"><div class="card-body">
        <form method="GET" class="row align-items-end ui-filter-bar">
            ...
            <details class="ui-filter-bar__more" @if(($filters['reference'] ?? '') !== '' || ($filters['keyword'] ?? '') !== '') open @endif><summary>{{ __('More Filters') }}</summary><div class="row mt-2"><div class="form-group col-md-4"><label>{{ __('Reference') }}</label><input name="reference" value="{{ $filters['reference'] ?? '' }}" class="form-control"></div><div class="form-group col-md-4"><label>{{ __('Search') }}</label><input name="keyword" value="{{ $filters['keyword'] ?? '' }}" class="form-control"></div></div></details>
            <div class="col-12 ui-filter-actions"><button class="btn btn-primary">{{ __('Apply filters') }}</button><a class="btn btn-light" href="{{ route('finance-transactions.index') }}">{{ __('Clear filters') }}</a></div>
        </form>
        <p class="ui-table-scroll-hint"><i class="fa fa-arrows-h" aria-hidden="true"></i> {{ __('Swipe horizontally to review every transaction column.') }}</p>
        <div class="table-responsive ui-responsive-list-wrap"><table class="table table-striped ui-responsive-list ui-mobile-cards"><thead><tr><th>{{ __('Date') }}</th><th>{{ __('Reference') }}</th><th>{{ __('Type') }}</th><th>{{ __('Student / Payee') }}</th><th>{{ __('Description') }}</th><th>{{ __('Payment Method') }}</th><th>{{ __('Fund Account') }}</th><th class="text-right">{{ __('Money In') }}</th><th class="text-right">{{ __('Money Out') }}</th><th>{{ __('Status') }}</th></tr></thead>
        <tbody>@forelse($rows as $row)<tr><td data-label="{{ __('Date') }}">{{ $row['date'] }}</td><td data-label="{{ __('Reference') }}"><span class="ui-cell-primary">{{ $row['reference'] ?: '-' }}</span></td><td data-label="{{ __('Type') }}">{{ $row['display_type'] ?: __(str_replace('_', ' ', $row['transaction_type'])) }}</td><td data-label="{{ __('Student / Payee') }}">{{ $row['counterparty'] }}</td><td data-label="{{ __('Description') }}">{{ $row['description'] }}</td><td data-label="{{ __('Payment Method') }}">{{ $row['payment_method'] }}</td><td data-label="{{ __('Fund Account') }}">{{ $row['fund_account'] }}</td><td data-label="{{ __('Money In') }}" class="text-right text-success">{{ $row['money_in'] ? number_format($row['money_in'], 2) : '-' }}</td><td data-label="{{ __('Money Out') }}" class="text-right text-danger">{{ $row['money_out'] ? number_format($row['money_out'], 2) : '-' }}</td><td data-label="{{ __('Status') }}"><span class="badge badge-success">{{ __('Completed') }}</span></td></tr>@empty<tr><td colspan="10" data-label=""><div class="ui-empty-state">{{ __('No transactions found.') }}</div></td></tr>@endforelse</tbody></table></div>
    </div></div>
</div>
