...
<div class="content-wrapper">
    ...
    <div class="card"><div class="card-body">
        ...
        <tbody>@forelse($rows as $row)<tr><td>{{ $row['date'] }}</td><td>{{ $row['reference'] ?: '-' }}</td><td>{{ $row['display_type'] ?: __(str_replace('_', ' ', $row['transaction_type'])) }}</td><td>{{ $row['counterparty'] }}</td><td>{{ $row['description'] }}</td><td>{{ $row['payment_method'] }}</td><td>{{ $row['fund_account'] }}</td><td class="text-right text-success">{{ $row['money_in'] ? number_format($row['money_in'], 2) : '-' }}</td><td class="text-right text-danger">{{ $row['money_out'] ? number_format($row['money_out'], 2) : '-' }}</td><td><span class="badge badge-success">{{ __('Completed') }}</span></td></tr>@empty<tr><td colspan="10" class="text-center">{{ __('No transactions found.') }}</td></tr>@endforelse</tbody></table></div>
    </div></div>
</div>
