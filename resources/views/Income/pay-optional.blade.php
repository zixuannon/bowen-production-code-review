                        ...
                        <form class="pt-3 create-form form-validation col-sm-12 col-md-5" method="post" action="{{ route('fees.optional.store') }}" novalidate="novalidate" data-success-function="formSuccessFunction">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-12 col-md-4">
                                    <label>{{ __('Deposit to Account') }} <span class="text-danger">*</span></label>
                                    @if($bankAccounts->isNotEmpty())
                                        {!! Form::select('bank_account_id', $bankAccounts->pluck('account_name', 'id'), null, ['class' => 'form-control', 'placeholder' => __('Select bank account'), 'required' => true]) !!}
                                    @else
                                        <div class="alert alert-warning">
                                            <i class="fa fa-exclamation-triangle"></i>
                                            {{ __('No active fund account is available. Please create a bank, cash, or mobile wallet account before recording a payment.') }}
                                        </div>
                                    @endif
                                </div>
                            </div>
