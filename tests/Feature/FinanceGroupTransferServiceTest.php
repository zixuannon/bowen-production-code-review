...
class FinanceGroupTransferServiceTest extends TestCase
{
    ...
    public function test_school_accountant_cannot_enter_group_operating_context_or_switch_schools(): void
    {
        [$group, , $schoolAccountant] = $this->groupFixture();

        $this->expectException(AuthorizationException::class);
        app(\App\Services\FinanceOperatingContextService::class)->enterSchool(
            User::on('mysql')->findOrFail($schoolAccountant->central_user_id), $group->id, 1,
        );
    }
    ...
    public function test_non_head_finance_cannot_open_group_funding_even_with_an_hq_assignment(): void
    {
        ...
        $this->expectException(AuthorizationException::class);
        app(\App\Http\Controllers\FinanceGroupTransferController::class)->funding($group);
    }
