...
class CentralFinanceStaffUuidMigrationRunnerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->trusted = ['SCH202615'=>$this->database(),'SCH202616'=>$this->database(),'SCH202619'=>$this->database()];
        $sqlite = static fn (string $database): array => ['driver'=>'sqlite','database'=>$database,'prefix'=>'','foreign_key_constraints'=>true];
        Config::set('database.connections.mysql', $sqlite($this->central)); Config::set('database.connections.school', $sqlite($this->trusted['SCH202615']));
        Config::set('finance_release.central_finance_staff_uuid_tenants', $this->trusted); Config::set('finance_release.central_finance_staff_uuid_canary', 'SCH202615');
        ...
        Config::set('database.connections.school.database', $this->trusted['SCH202615']); DB::purge('school');
    }
    ...
    {
        $this->assertSame(['SCH202615','SCH202616','SCH202619','SCH202620','SCH202621','SCH202631','SCH202632'], array_keys(MigrateCentralFinanceStaffUuid::PRODUCTION_TENANTS));
    ...
    public function test_default_is_zero_write_and_database_names_demo_and_unknown_codes_are_rejected(): void
    {
        ...
        $this->artisan('finance:migrate-central-finance-staff-uuid',['--tenant'=>[$this->trusted['SCH202615']]])->assertExitCode(1);
    ...
    {
        $this->assertNotSame($this->trusted['SCH202615'], $this->trusted['SCH202616']);
        $this->assertFalse($this->column($this->trusted['SCH202615']));
        $this->assertSame([], $this->migrations($this->trusted['SCH202615']));
        ...
        $this->artisan('finance:migrate-central-finance-staff-uuid',['--tenant'=>['SCH202615'],'--execute'=>true])->assertExitCode(0);
        $this->assertTrue($this->column($this->trusted['SCH202615']));
        $this->assertSame([MigrateCentralFinanceStaffUuid::MIGRATION], $this->migrations($this->trusted['SCH202615']));
    ...
    {
        $this->on($this->trusted['SCH202615'], fn () => Schema::connection('school')->table('users', fn ($table) => $table->uuid('central_finance_source_uuid')->nullable()));
        $this->artisan('finance:migrate-central-finance-staff-uuid',['--tenant'=>['SCH202615']])->assertExitCode(1);
        $this->assertSame([], $this->migrations($this->trusted['SCH202615']));
    }
