...
if grep -Eq "header\(['\"]x-(forwarded|original)-host|server\(['\"]HTTP_X_(FORWARDED|ORIGINAL)" \
  "$release_dir/app/Http/Controllers/Controller.php"; then
  echo "GUARD_FAIL: unrestricted forwarded-host fallback detected" >&2
  exit 1
fi
